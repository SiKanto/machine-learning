...
2. Belum adanya sistem yang secara efektif memanfaatkan data detail destinasi (seperti rating, fasilitas, deskripsi, dan nama kota) untuk memberikan rekomendasi yang personal dan terarah. Wisatawan sering kali bergantung pada sumber informasi yang terbatas dan kurang tepat untuk memilih destinasi yang sesuai dengan keinginan mereka.
...
1. Membangun sebuah sistem rekomendasi destinasi wisata di Madura yang personal dan relevan berdasarkan informasi detail destinasi (seperti rating, kota, fasilitas, dan harga tiket), untuk memudahkan wisatawan dalam memilih destinasi wisata sesuai dengan preferensi pribadi mereka.
...
Untuk mencapai tujuan yang telah ditetapkan, pendekatan machine learning yang digunakan adalah pembangunan model klasifikasi untuk kota destinasi wisata. Berikut adalah pendekatan yang akan digunakan dalam proyek ini:
* Proyek ini menggunakan model Multilayer Perceptron (MLP) yang diimplementasikan dengan TensorFlow dan Keras. Model ini dipilih untuk melakukan klasifikasi kota destinasi wisata berdasarkan fitur-fitur yang relevan.
* Model ini akan memproses fitur teks (kolom "asal kabupaten") menggunakan TF-IDF, menormalisasi fitur numerik (rating, fasilitas, jumlah pengunjung, harga tiket), dan menggunakan Label Encoder untuk kota wisata. Proses ini memungkinkan model untuk mempelajari pola dan hubungan antar fitur dalam dataset destinasi.
* Model yang telah dilatih akan digunakan untuk memprediksi atau mengklasifikasikan kota destinasi berdasarkan input yang diberikan (seperti nama kota dan fitur numerik lainnya).
...
A. Model Klasifikasi dengan Multilayer Perceptron (MLP)
  ...
  * Arsitektur dan Proses Model MLP:
    ...
    * Output Layer: Merupakan dense layer terakhir yang memiliki jumlah unit sama dengan jumlah kota wisata unik yang ada dalam dataset. Fungsi aktivasi yang digunakan adalah softmax (softmax), yang menghasilkan distribusi probabilitas di atas semua kota. Probabilitas tertinggi menunjukkan kota yang paling mungkin untuk destinasi dengan fitur input tersebut.
  ...
  * Kelebihan Model MLP untuk Klasifikasi Kota:
    * MLP dapat menangkap pola dan interaksi kompleks antar fitur yang mungkin tidak linier, sehingga menghasilkan klasifikasi kota yang lebih akurat.
    * Model dapat beradaptasi dengan berbagai jenis input (numerik dan hasil vektorisasi teks) setelah pra-pemrosesan yang tepat.
    * Setelah dilatih, model MLP dapat memberikan prediksi kota dengan sangat cepat, yang penting untuk sistem rekomendasi real-time.
  ...
  * Kekurangan Model MLP untuk Klasifikasi Kota:
    * Model ini memerlukan label kota yang jelas dan akurat untuk setiap destinasi selama pelatihan.
    Tidak Langsung Memberikan Rekomendasi Item: Model ini hanya mengklasifikasikan kota destinasi. Untuk memberikan rekomendasi item spesifik (nama wisata), dibutuhkan langkah tambahan yaitu memfilter dataset berdasarkan kota yang diprediksi.
...
Setelah model MLP berhasil dilatih dan disimpan, tahap selanjutnya adalah menggunakannya untuk memberikan rekomendasi kepada wisatawan. Proses ini melibatkan prediksi kota dan kemudian penyaringan destinasi.
  * Proses Prediksi:
    ...
    * Inferensi Model: Fitur input yang sudah dipra-proses kemudian dimasukkan ke dalam model MLP yang telah dilatih. Model akan menghasilkan probabilitas untuk setiap kota wisata.
    * Penentuan Kota Prediksi: Kota dengan probabilitas tertinggi akan dipilih sebagai kota destinasi yang diprediksi sesuai dengan preferensi input pengguna.
    * Dekode Kota: Label kota yang telah di-encode akan diubah kembali menjadi nama kota yang dapat dimengerti (misalnya, dari 0 menjadi Bangkalan) menggunakan LabelEncoder yang sama.
    
  * Penyajian Top-N Recommendation:
    Setelah kota destinasi diprediksi, langkah selanjutnya adalah menyajikan rekomendasi destinasi spesifik kepada pengguna.
    * Dataset asli destinasi wisata Madura akan difilter untuk menampilkan hanya destinasi yang termasuk dalam kota yang diprediksi.
    * Destinasi dalam kota yang diprediksi dapat diurutkan berdasarkan kriteria tambahan seperti rating (tertinggi), jumlah_pengunjung, atau harga_tiket (terendah) untuk memberikan rekomendasi yang lebih optimal.
  ...
  * Kelebihan Top-N Recommendation sebagai Output:
    ...
    * Rekomendasi didasarkan pada kota yang diprediksi sesuai preferensi input pengguna.
  
  * Kekurangan Top-N Recommendation:
    * Jika prediksi kota oleh model kurang akurat, rekomendasi Top-N juga akan kurang relevan.
    * Pendekatan ini mungkin membatasi pengguna pada destinasi dalam kota yang diprediksi, mengurangi peluang untuk menemukan destinasi menarik di luar kota tersebut.
...
## Evaluation
Pada bagian Evaluasi, akan dijelaskan metrik evaluasi yang digunakan untuk menilai kinerja model sistem rekomendasi destinasi wisata di Pulau Madura. Karena model yang dibangun adalah model klasifikasi Multilayer Perceptron (MLP) untuk memprediksi kota wisata, metrik evaluasi yang relevan adalah Akurasi dan Loss (Sparse Categorical Crossentropy).

A. Akurasi (Accuracy)
Akurasi adalah metrik evaluasi utama yang digunakan untuk mengukur kinerja model klasifikasi. Akurasi menghitung proporsi prediksi yang benar (yaitu, berapa banyak kota destinasi wisata yang diprediksi dengan benar oleh model) dari total jumlah prediksi. Alasan penggunaan Accuracy karena merupakan metrik langsung dan mudah dipahami untuk mengukur persentase prediksi kota yang benar oleh model.

B. Sparse Categorical Crossentropy (Loss Function)
Sebagai fungsi loss dalam proses pelatihan model klasifikasi MLP, digunakan Sparse Categorical Crossentropy. Fungsi loss ini mengukur "kesalahan" atau "jarak" antara probabilitas kota yang diprediksi oleh model dengan label kota yang sebenarnya (dalam format integer, seperti yang dihasilkan oleh Label Encoder). Model berusaha meminimalkan nilai loss ini selama pelatihan. Alasan penggunaan Loss Function karena model mengklasifikasikan ke dalam banyak kota (multi-kelas) dan label kota telah diubah menjadi format integer (setelah Label Encoding), membuatnya efisien dan tepat untuk melatih neural network pada tugas ini.
